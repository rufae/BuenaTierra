...
public class ProduccionController : ControllerBase
{
    ...
    /// <summary>GET /api/produccion/hoy — Producciones del día con nombre de producto y lote</summary>
    [HttpGet("hoy")]
    public async Task<ActionResult<ApiResponse<object>>> GetHoy(CancellationToken ct)
    {
        ...
        var dto = producciones.Select(p => new
        {
            p.Id,
            p.EmpresaId,
            p.ProductoId,
            ...
            // Lote generado (si ya se finalizó)
            codigoLote = p.Lotes.FirstOrDefault()?.CodigoLote,
            loteId = p.Lotes.FirstOrDefault()?.Id,
            fechaCaducidad = p.Lotes.FirstOrDefault()?.FechaCaducidad,
        });
